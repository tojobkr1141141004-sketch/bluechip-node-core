...
export function UserSectionPage({
  eyebrow,
  title,
  description,
  nextPhase
  ...
  return (
    <section className="rounded-3xl border border-white/[0.07] bg-white/[0.025] p-6 sm:p-8">
      ...
      <div className="mt-8 max-w-2xl rounded-2xl border border-amber-300/10 bg-amber-300/[0.04] p-5">
        ...
        <div className="mt-3 text-sm font-semibold">화면 골격 준비 완료</div>
        <p className="mt-1.5 text-xs leading-5 text-slate-500">
          실제 처리 로직은 {nextPhase}에서 데이터 원장과 연결합니다.
        </p>
      </div>

      <Link
        href="/dashboard"
        className="mt-6 inline-flex items-center gap-2 rounded-xl border border-white/10 px-4 py-2.5 text-xs font-semibold text-slate-300 hover:bg-white/[0.04]"
      >
